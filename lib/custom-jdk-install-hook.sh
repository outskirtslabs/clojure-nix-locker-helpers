...
#
#  Env variables:
#  - jarPath: Path to a uberjar. If set, jdeps analyzes it to pick the JDK
#             modules and a wrapper binary is generated. (OPTIONAL)
#  - jdkModules: Option passed to jlink --add-modules. If null, jdeps will be
#                used to analyze the jar and pick the necessary modules
...
customJdkInstallHook() {
  ...
  if [ -n "${jarPath-}" ]; then
    mkdir -p "$out/bin"
    binaryPath="$out/bin/${binaryName-${name-customJdk}}"
